...
This unexpected behavior is not specific to x86. In the x86 example above, the behavior can
be explained by store buffers. More generally, unexpected shared-memory behaviors arise
because architectures may delay, reorder, or make memory operations visible in different
ways. Other architectures, such as ARM and RISC-V, can allow even more subtle behaviors
in their shared-memory designs~\cite{10.1145/3458926ArmedCat,riscv-spec,DennisPhdThesis}.
Thus, even if we start from the same source program, compiling and running it on different
architectures may produce different observable behaviors.
...
If we follow the edges in Figure~\ref{fig:litmus-execution-0-0} starting from $W(X)=1$ in Thread 0, we can trace a path through the graph: the \textit{po} edge tells us that $R(Y)=0$ is executed after $W(X)=1$; the \textit{fr} edge 
then tells us that $W(Y)=1$ must be executed after $R(Y)=0$; the \textit{po} edge in Thread 1 tells us that $R(X)=0$ is executed after $W(Y)=1$; and finally, the \textit{fr} edge brings us back to $W(X)=1$, meaning it must be executed after $R(X)=0$. We have arrived at a contradiction: $W(X)=1$ in Thread 0 must be executed both before and after itself. This is impossible in any consistent ordering. In Figure~\ref{fig:litmus-execution-0-0}, we obtain a cycle:
\[
W(X)=1 \xrightarrow{po} R(Y)=0 \xrightarrow{fr} W(Y)=1 \xrightarrow{po} R(X)=0 \xrightarrow{fr} W(X)=1.
\]

This gives us a natural rule: if we can find a cycle in the execution graph, the candidate execution is contradictory and should be forbidden. The arrows in an execution graph can be viewed as binary relations between events. A binary relation describes a connection between two things, such as two instructions in an execution. Therefore, many questions about memory consistency models can be treated as mathematical questions about binary relations, such as whether a relation is acyclic or whether one set of allowed executions is included in another. We will explain binary relations in detail in Chapter~\ref{chapter:Background}.

The example above shows that memory consistency models are not only useful for explaining
individual litmus-test outcomes. We now explain these broader motivations in more detail.

\section{Motivation: Memory Consistency Models}

A memory consistency model describes the shared-memory behaviors allowed by an architecture~\cite{lam1675439,sewell2010x86tso}. Each architecture may have its own way to interact with shared memory. When different architectures are used together, or when software is moved from one architecture to another, we need to understand how their memory behaviors relate to each other. Many problems in heterogeneous systems can therefore be understood as problems about memory consistency models.
...
All four of these problems share a common requirement: we need precise, general descriptions of memory consistency models that go beyond what empirical testing can provide.

\section{Formal Reasoning About Memory Models}

Formal methods describe program executions using mathematical structures, such as binary relations, and by imposing axioms on these structures~\cite{10.1145/2627752,10.1145/3458926ArmedCat}. Binary relations can be studied mathematically, and many existing theorems about relations can be used to reason about memory consistency models~\cite{rosen2012discrete}. 

Formal memory consistency models not only explain why a specific behavior can happen on one architecture, but also compare the behaviors allowed by different architectures. This is important because modern systems often involve multiple architectures, and the same concurrent program may behave differently on different hardware. Therefore, we need tools that can support both concrete reasoning about one architecture and general comparison between architectures.
...
\section{From Cat Specifications to \textsc{LeanCat}}
...
For example, a small Cat fragment can look like this:
...
Intuitively, each relation is a set of arrows in the execution graph. The first line says that \texttt{com} contains all arrows from \texttt{rf}, \texttt{fr}, and \texttt{co}; in other words, the union operator \texttt{|} connects these edge sets together into one larger edge set. The second line says that, after we also add the program-order arrows \texttt{po}, we must not be able to follow the arrows around a cycle and return to the same event. If such a cycle exists, the candidate execution is rejected.

Cat is therefore a useful starting point for mechanized reasoning. Instead of rewriting memory consistency models from scratch in a proof assistant, we can reuse existing Cat specifications. However, Cat specifications themselves are not machine-checked proofs. They describe the allowed executions of a memory consistency model, but they do not by themselves provide a proof infrastructure for proving general properties about memory consistency models, such as model inclusion, preservation across updates, or correctness of translations between architectures. These questions can be stated mathematically, and in principle, they can be proved by pen-and-paper reasoning. However, pen-and-paper proofs are difficult to check, easy to get wrong, and hard to maintain when memory consistency models evolve.

To address this problem, we use interactive theorem proving (ITP). An ITP allows us to write mathematical definitions and proofs that are checked by a machine. Mechanization in ITP requires additional work, because definitions and proofs must be encoded in the proof assistant. However, it also provides important benefits: it reduces mistakes, makes definitions and lemmas reusable, and makes proofs easier to maintain when memory consistency models change.

In this work, we use Lean~4 and Mathlib. Lean~4 allows us to mechanize memory consistency model definitions and proofs, so that the proof assistant can check the correctness of our reasoning~\cite{deMoura2021lean4}. Lean~4 is a good fit for this project because it also provides a powerful metaprogramming framework~\cite{10.1145/3110278Lean4Meta}. This is important because \textsc{LeanCat} needs to read Cat specifications and generate the corresponding Lean definitions automatically, instead of requiring users to encode every memory model by hand. Mathlib provides a rich library of reusable mathematical definitions and lemmas, giving us a strong foundation for reasoning about relations, orders, sets, and other mathematical structures used in memory consistency models~\cite{mathlib2020}.

\subsection{Contributions}

Finally, we introduce \textsc{LeanCat}, a compiler and proof infrastructure that takes memory models written in Cat and translates them into binary relations in Lean~4. We make the following contributions. First, we implement a Cat-to-Lean~4 compiler that parses existing Cat specifications and generates the corresponding Lean~4 definitions, so that existing Cat memory consistency models do not need to be rewritten from scratch. Second, we provide a proof infrastructure that allows users to prove properties about memory consistency models, such as whether one memory consistency model is weaker than another; when a memory consistency model changes, \textsc{LeanCat} can regenerate the Lean~4 definitions and help check the related properties again. Third, we demonstrate that \textsc{LeanCat} supports both concrete and general reasoning: it can check whether a specific litmus test is allowed under a particular architecture, and it can also be used to prove more general properties between different architectures. Finally, we integrate execution graph visualization into the Lean~4 development environment, providing interfaces that allow users to inspect the execution graph of a litmus test and understand why a behavior is allowed or forbidden.