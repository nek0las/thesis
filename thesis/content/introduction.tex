...
Here, the first line defines a relation \texttt{com} by taking the union of reads-from (\texttt{rf}), from-read (\texttt{fr}), and coherence order (\texttt{co}). The second line states an acyclicity axiom: if the union of program order (\texttt{po}) and communication edges has a cycle, then the candidate execution is rejected. This example is simplified, but it gives the basic shape of Cat: define relations, combine them with relational operators, and state constraints over the resulting graph.
